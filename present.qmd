...
Designed for landscape-scale analysis, LANDFIRE datasets are delivered at 30M resolution.  Below we summarize to 10K acre hexagons for ease of viewing and to better match intended scale of use.

## Map of prevalent existing vegetation types, height and cover 
...
# --- EVT palette: use the provided name -> hex mapping ---
evt_palette_named <- c(
  "Developed-Roads"                                        = "#999999",
  "Eastern Cool Temperate Fallow/Idle Cropland"            = "#EFEEC5",
  "Eastern Cool Temperate Pasture and Hayland"             = "#E8E5A4",
  "Eastern Cool Temperate Row Crop"                        = "#CDC90F",
  "Eastern Warm Temperate Row Crop"                        = "#9A970B",
  "North-Central Interior Dry-Mesic Oak Forest and Woodland" = "#70B65F",
  "North-Central Interior Floodplain Forest"               = "#C4BBD5",
  "Open Water"                                             = "#D7EDF7",
  "Ozark-Ouachita Dry-Mesic Oak Forest"                    = "#92C484",
  "South-Central Interior Large Floodplain Forest"         = "#9B89B4",
  "South-Central Interior Mesophytic Forest"               = "#00703C",
  "South-Central Interior Small Stream and Riparian Forest" = "#5E3D7F",
  "Southeastern Ruderal Grassland"                         = "#3CABB5",
  "Southeastern Ruderal Wet Meadow & Marsh"                = "#351150",
  "Southern Interior Low Plateau Dry-Mesic Oak Forest"     = "#00381E"
)

# Domain is the set of EVT names in your data (sorted like your original)
evt_names <- sort(unique(hexs$evt_name))

# Flag EVT names in data that do NOT have a provided color
evt_unmapped <- setdiff(evt_names, names(evt_palette_named))
if (length(evt_unmapped) > 0) {
  message("EVT names without a provided color: ", paste(evt_unmapped, collapse = "; "))
}

# Build a palette vector aligned to the domain.
# For EVT names missing in the provided palette, fall back to no_data_color.
evt_colors <- vapply(
  evt_names,
  function(nm) if (nm %in% names(evt_palette_named)) evt_palette_named[[nm]] else no_data_color,
  FUN.VALUE = character(1)
)

# Leaflet categorical palette for EVT (same interface as your working code)
pal_evt <- colorFactor(palette = evt_colors, domain = evt_names)

# -------------------------
# 2) Build EVC/EVH hex lookups and attach to sf (no joins)
# -------------------------

# Helper: make a named hex vector from a ramp table keyed by Value (integer)
# Assumes ramp tables have columns: Value, R, G, B (0–255). If they have extra columns
# (e.g., CLASSNAMES), they are ignored here. -9999 is mapped to no_data_color.
to_hex_map <- function(ramp_tbl, no_data_hex = no_data_color) {
  vals <- as.integer(ramp_tbl$Value)
  hexs <- rgb(ramp_tbl$R, ramp_tbl$G, ramp_tbl$B, maxColorValue = 255)
  hexs[vals == -9999] <- no_data_hex
  # return a named vector: names are the value codes
  names(hexs) <- vals
  hexs
}

evc_hex_map <- to_hex_map(evc_ramp, no_data_hex = no_data_color)
evh_hex_map <- to_hex_map(evh_ramp, no_data_hex = no_data_color)

# Attach hexs directly, preserving your existing label columns (evc_labels, evh_labels)
hexs2 <- hexs %>%
  mutate(
    ...
    evc_hex   = ifelse(
      ...
      no_data_color,
      ifelse(
        evc_value %in% as.integer(names(evc_hex_map)),
        evc_hex_map[as.character(evc_value)],
        no_data_color
      )
    ),
    evh_hex   = ifelse(
      ...
      no_data_color,
      ifelse(
        evh_value %in% as.integer(names(evh_hex_map)),
        evh_hex_map[as.character(evh_value)],
        no_data_color
      )
    )
  )
...
evc_missing <- setdiff(unique(hexs2$evc_value), as.integer(names(evc_hex_map)))
evh_missing <- setdiff(unique(hexs2$evh_value), as.integer(names(evh_hex_map)))
...
leaflet(hexs2) %>%
  ...
  # Layer A: EVT (Existing Vegetation Type) - categorical palette using provided hexes
  addPolygons(
  ...
  addPolygons(
    ...
    label = ~paste0("Height: ", evh_labels),
    labelOptions = labelOptions(
      style = list("font-weight" = "normal", padding = "3px 8px"),
      textsize = "15px", direction = "auto"
    ),
