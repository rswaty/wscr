...
LANDFIRE's Biophysical Settings (BpSs) are the source data for the maps and charts below. Designed for landscape-scale analysis, LANDFIRE datasets are delivered at 30M resolution.  Below we summarize to 10K acre hexagons for ease of viewing and to better match intended scale of use. 
...
Use this page as a starting point to explore the most prevalent BpSs and historical fire regimes for your analysis area. In addition to the charts and maps, the historical disturbance data can be directly accessed using this site.
...
* NoData values in the Fire Return Interval layer typically indicate an "Open Water" underlying BpS, or barren ground. 
* the FRI(years) value in the popup for the Fire Return Interval represents the mean number of years that would have passed between fires for a site for all fire types, surface, mixed or replacement.  For example, a value of 17 indicates that on average there would have been a fire of some type every 17 years.
...
# --- FRI palette using custom bins/colors (from your table) ---
no_data_color <- "#4D4D4D"

# Breaks are right-closed (prev, break]; adjust `right = FALSE` if you prefer [prev, break)
fri_breaks <- c(-Inf, 5, 15, 35, 100, 200, 500, 1300, Inf)

# 8 colors to match 8 intervals; duplicate the last color for the overflow bin (>=1300)
fri_colors <- c("#BF4A3F", "#F7AC74", "#FFF2A8", "#D5E6CF", "#CED8ED", "#7D9FD1", "#1C76BA", "#1C76BA")
...
# --- Legend labels exactly matching breaks ---
fri_labels <- c(
  "≤ 5",
  "5–15",
  "15–35",
...
json_output <- toJSON(bbox_list, keep_vec_names = TRUE)  # if needed elsewhere
...
m <- leaflet(hexs) %>%
  ...
  addPolygons(
    ...
    group = "Historical Ecosystems"
  ) %>%
  
  # Layer 2: FRI polygons (custom bins/colors; -9999/NA -> dark gray)
  addPolygons(
    fillColor = ~ifelse(
      fri_allfir == -9999 | is.na(fri_allfir),
      no_data_color,
      pal_fri(fri_allfir)
    ),
    color = "#BDBDC3",
...
# 
# # --- FRI Legend (explicit labels & colors) ---
# m <- m %>%
#   addLegend(
#     position = "bottomright",
#     colors   = fri_colors,  # length 8
#     labels   = fri_labels,  # length 8
#     title    = "FRI (years)",
#     opacity  = 1
#   ) %>%
#   addLegend(
...
```{r bps chart, message=FALSE, warning=FALSE, echo=FALSE, fig.width=10, fig.height=11}
...
```{r fire bar, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=11}
...
# Plot
 fri_chart <- 
  ...
  labs(
    ...
    subtitle = "Fire types: Replacement = 75 - 100% top kill, Mixed = 26 - 74%, Surface = 0 - 25% \nPercentage of landscape in BpS in parentheses; X-axis numbers represent average number of years between fires.",
    caption = "\nData from landfire.gov.",
...
* The names below are followed by a number in parentheses.  The first 5-digit number is the BPS Code.  The following numbers reprent the Map Zones.  For example, "13020_65_66" is ‘Laurentian-Acadian Northern Hardwoods forest’ BpS (13020) in Map Zones 65 and 66 (example only; may or may not be found in this landscape).
